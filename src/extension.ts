...
function findLastQuote(input: string, position: number,findChar:string): number {
	let inEscape = false;
...
function findNextQuote(input: string, position: number,findChar:string): number {
	let inEscape = false;
...
export function activate(context: vscode.ExtensionContext) {
	...
	let disposable = vscode.commands.registerCommand('string-copier.copyString', (uri?: vscode.Uri) => {
		...
		const stringContent = (()=>{
			...
			for(let c of char){
				if(ranges[c].start!==-1 && ranges[c].end!==-1){
					...
					return lineText.substring(ranges[c].start,ranges[c].end);
					break;
				}
			}
			return '';
